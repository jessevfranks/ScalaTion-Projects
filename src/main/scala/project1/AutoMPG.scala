...
// Code used from Example_AutoMPG
object autoMPG:
  ...
  @main def autoMPGrun(): Unit =
    val utilsA = new Project1Utils(autoMPG.ox, autoMPG.y, autoMPG.ox_fname)

    utilsA.runLinearRegressions()
    // utilsA.runRidgeRegressions()
    // utilsA.runLassoRegressions()
    // utilsA.runTransformedRegressions()
    // utilsA.runSymRidgeRegression()

    // utilsA.runForwardSelect()
    // utilsA.runBackwardsElimination()
    // utilsA.runStepwiseSelect()
